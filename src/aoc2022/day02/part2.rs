...
fn play(scope: &PuzzleScope, puzzle: &str) -> i32 {
    ...
    for v in lib::io::read_puzzle_as_list(scope.day(), puzzle) {
        ...
        let player_select = if player_select_strategy == 'X' {
            opponent_select.require_for_lost()
        } else if player_select_strategy == 'Y' {
            opponent_select
        } else if player_select_strategy == 'Z' {
            opponent_select.require_for_win()
        } else {
